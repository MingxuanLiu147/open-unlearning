...
class VLKEBDataset(MMEditingDataset):
    """VLKEB dataset (NeurIPS 2024 Datasets & Benchmarks Track).

    A large vision-language model knowledge editing benchmark with
    knowledge-graph-based multi-hop portability evaluation.

    Paper: https://arxiv.org/abs/2403.07350
    HuggingFace: HymanH/VLKEB-data

    Fields follow the same convention as MMEdit VQA, plus ``port_new``
    for 1-4 hop portability::

        {
          "image": "m.0104lr/google_15.jpg",
          "src": "...", "pred": "...", "alt": "...",
          "rephrase": "...", "image_rephrase": "...",
          "loc": "...", "loc_ans": "...",
          "m_loc": "...", "m_loc_q": "...", "m_loc_a": "...",
          "port_new": [
            {"port_q": "...", "port_a": "...",
             "port_edit_q": "...", "port_edit_a": "..."},
            ...
          ]
        }
    """
    ...
    SUBSET_FILES = {
        "train": "data/edit/vlkeb/train.json",
        "eval": "data/edit/vlkeb/eval.json",
        "eval_multihop": "data/edit/vlkeb/eval_multihop.json",
        "eval_edit_onehop": "data/edit/vlkeb/eval_edit_onehop.json",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hf_args is None and data_path is None:
            default_hf = {
                "path": "HymanH/VLKEB-data",
                "split": self.subset.replace("_", "."),
            }
        super().__init__(
            data_path=data_path,
            image_dir=image_dir,
    ...
    def _default_data_path(self, split: Optional[str]) -> Optional[str]:
        return self.SUBSET_FILES.get(self.subset, self.SUBSET_FILES["eval"])
    ...
    def normalize_record(
        ...
        portability_inputs = self._build_portability(item.get("port_new"))
    ...
    @staticmethod
    def _build_portability(port_new) -> Optional[Dict[str, List[Dict[str, str]]]]:
        if not port_new or not isinstance(port_new, list):
            return None
        entries: List[Dict[str, str]] = []
        for hop_idx, hop in enumerate(port_new):
            if not isinstance(hop, dict):
                continue
            q = hop.get("port_edit_q") or hop.get("port_q", "")
            a = hop.get("port_edit_a") or hop.get("port_a", "")
            if q and a:
                entries.append({"prompt": str(q).strip(), "ground_truth": str(a).strip()})
        if not entries:
            return None
        return {"multihop_portability": entries}
